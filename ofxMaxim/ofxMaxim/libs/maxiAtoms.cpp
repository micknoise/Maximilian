...
#endif
#include <algorithm>
#define USEVFORCE 0 //apple systems
...
#if USEVFORCE && defined(__VFORCE_H)
...
void maxiAtomBookPlayer::queueAtomsBetween(maxiAccelerator &atomStream, long start, long end, int blockOffset) {
    ...
    const float speedMod = playbackSpeed == 0 ? 0 : 1.0 / playbackSpeed;
...
//            cout << thisAtomIndex  << ", ";
        ...
        if (rand() / static_cast<float>(RAND_MAX) < probability) {
            ...
            if (frequencyEnvelopeOn) {
                const float linFreq = maxiMap::linlin(freq, 20, 20000, 0, frequencyEnvelope.size()-1);
                const float ampScale = frequencyEnvelope[static_cast<int>(floor(linFreq))];
                amp *= ampScale;
            }
...
//                    atomStream.addAtom(freq, atom->phase, 44100, atom->length * lengthMod, amp, (blockOffset + atomPos - loopedSamplePos) * speedMod);
                    atomCount++;
                }
           }
        }
...
maxiAtomBookPlayer &maxiAtomBookPlayer::setFrequencyEnvelope(int size, float * bins) {
    if (frequencyEnvelope.size() != size) {
        frequencyEnvelope.resize(size);
    }
    for(int i=0; i < size; i++) {
        frequencyEnvelope[i] = bins[i];
    }
}
